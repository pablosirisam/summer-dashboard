...
const ROWS: RowCfg[] = [
  { key: 'ia',    icon: Brain,    color: '#f0c26a', glow: '217,164,65',  entry: 'ia_entry',    done: 'ia_completed',    tag: l => l.ia_hours ? `${l.ia_hours}h` : '' },
  { key: 'food',  icon: Salad,    color: '#9dc587', glow: '127,169,107',  entry: 'food_entry',  done: 'food_completed',  tag: l => l.food_rating ? `${l.food_rating}★` : '' },
  { key: 'sport', icon: Dumbbell, color: '#e68463', glow: '210,98,74',  entry: 'sport_entry', done: 'sport_completed', tag: l => l.sport_minutes ? `${l.sport_minutes}min` : '' },
]
    ...
    <div className="timeline">
      {items.map((log, i) => {
        ...
        if (single) {
          ...
          node = n >= 3 ? '#9dc587' : n === 2 ? '#e68463' : n === 1 ? '#f0c26a' : '#697089'
          nodeGlow = n >= 3 ? '127,169,107' : n === 2 ? '210,98,74' : n === 1 ? '217,164,65' : '105,112,137'
        }
        return (
          <motion.div
