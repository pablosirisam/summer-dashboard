...
const ROWS: {
  ...
  { icon: Brain,    color: '#818cf8', glow: '99,102,241',  entry: 'ia_entry',    done: 'ia_completed',    tag: l => l.ia_hours ? `${l.ia_hours}h` : '' },
  { icon: Salad,    color: '#34e6a0', glow: '16,217,139',  entry: 'food_entry',  done: 'food_completed',  tag: l => l.food_rating ? `${l.food_rating}★` : '' },
  { icon: Dumbbell, color: '#fbbf24', glow: '251,146,60',  entry: 'sport_entry', done: 'sport_completed', tag: l => l.sport_minutes ? `${l.sport_minutes}min` : '' },
]
  ...
  return (
    <div className="timeline">
      {items.map((log, i) => {
        ...
        const node = n >= 3 ? '#34e6a0' : n === 2 ? '#fbbf24' : n === 1 ? '#818cf8' : '#697089'
        const nodeGlow = n >= 3 ? '16,217,139' : n === 2 ? '251,146,60' : n === 1 ? '99,102,241' : '105,112,137'
        return (
          <motion.div
